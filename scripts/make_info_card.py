...
def main():
    ...
    for idx, item in enumerate(lines):
        ...
        if item_type == 'header':
            ...
            svg_elements.append(f'    <text x="22" y="{y_pos}" class="title">ar1es-xd</text>')
            svg_elements.append(f'    <text x="96" y="{y_pos}" class="val">@</text>')
            svg_elements.append(f'    <text x="108" y="{y_pos}" class="title">github</text>')
            svg_elements.append(f'  </g>')
        
        elif item_type == 'divider':
            svg_elements.append(f'  <g class="animate-line line-{idx}">')
            svg_elements.append(f'    <text x="22" y="{y_pos}" class="divider">{html_escape(item[1])}</text>')
            svg_elements.append(f'  </g>')
            
        elif item_type == 'kv':
            key, val = item[1]
            svg_elements.append(f'  <g class="animate-line line-{idx}">')
            svg_elements.append(f'    <text x="22" y="{y_pos}" class="key">{html_escape(key)}:</text>')
            svg_elements.append(f'    <text x="110" y="{y_pos}" class="val">{html_escape(val)}</text>')
            svg_elements.append(f'  </g>')

        elif item_type == 'quote':
            q1, q2 = item[1]
            svg_elements.append(f'  <g class="animate-line line-{idx}">')
            svg_elements.append(f'    <text x="22" y="{y_pos}" class="quote">{html_escape(q1)}</text>')
            svg_elements.append(f'    <text x="22" y="{y_pos + 17}" class="quote">{html_escape(q2)}</text>')
            svg_elements.append(f'  </g>')

        elif item_type == 'colors':
            y_pos_colors = y_pos + 18
            color_blocks = [
            ...
            for c_idx, color in enumerate(color_blocks):
                x_pos = 22 + c_idx * 28
                svg_elements.append(f'    <rect x="{x_pos}" y="{y_pos_colors}" width="22" height="14" fill="{color}" rx="2"/>')
            svg_elements.append(f'  </g>')

    # SVG layout
    svg_content = f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {height}" width="100%" height="100%">
  <style>
    .title {{ font-family: 'Fira Code', monospace; font-size: 14.5px; font-weight: bold; fill: #10b981; }}
    .key {{ font-family: 'Fira Code', monospace; font-size: 12.5px; font-weight: bold; fill: #10b981; }}
    .val {{ font-family: 'Fira Code', monospace; font-size: 12.5px; fill: #cbd5e1; }}
    .divider {{ font-family: 'Fira Code', monospace; font-size: 12.5px; fill: #30363d; }}
